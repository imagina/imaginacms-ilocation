...
class EloquentLocatableRepository extends EloquentCoreRepository implements LocatableRepository
{
  /**
  ...
  public function beforeCreate(array &$data): void
  {
    if (!isset($data["system_name"])) {
      $entityType = $data['entity_type'] ?? '__global__';
      $entityId = $data['entity_id'] ?? 0;

      $data["system_name"] = strtolower(str_replace(['\\', '/'], '_', $entityType) . '_' . $entityId);
    }
  }
}
